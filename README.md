...
現在の実装は、v0.2 Runtime Hardeningとして安全な初期化、検証基盤、bounded discovery coverage gate、Google Sheetsからのread-only snapshot exportを追加した状態です。CakePHP AST解析器とPlaywright生成器は後続実装です。
...
release note草案は `docs/RELEASE_NOTES_v0.2.0.md`、手順確認は `docs/RELEASE_CHECKLIST.md` にあります。このrepoはPhase 5B時点ではtag作成、GitHub Release作成、PyPI publish、Docker publishを自動化しません。
