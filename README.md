...
現在の実装は、v0.2 Runtime Hardeningとして安全な初期化、検証基盤、bounded discovery coverage gateを追加した状態です。CakePHP AST解析器とPlaywright生成器は後続実装です。
...
- Google Sheetsからのread-only coverage snapshot export
...
Coverage gate評価はPhase 4A時点ではローカルJSON入力だけを使います。Google Sheets APIへread/writeしません。
