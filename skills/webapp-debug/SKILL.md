...
Google Sheets初期化は `scripts/init_sheets.py` を使う。実Google Sheets API統合テストは明示envが揃った場合だけ実行する。CakePHP AST解析器、Playwright Scenario生成器、coverage evaluatorは後続実装であり、現時点で実装済みと扱わない。
