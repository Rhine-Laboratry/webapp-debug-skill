...
Google Sheets初期化は `scripts/init_sheets.py` を使う。coverage gate評価は `inventory-json` 入力を使う。Sheetsの現在状態を評価する場合は、`scripts/export_sheets_snapshot.py` でread-only snapshotを作成してから `scripts/evaluate_coverage.py` へ渡す。snapshot exportはSheets write、lock、WAL、bootstrap、config更新を行わない。実Google Sheets API統合テストは明示envが揃った場合だけ実行する。CakePHP AST解析器、Playwright Scenario生成器は後続実装であり、現時点で実装済みと扱わない。
