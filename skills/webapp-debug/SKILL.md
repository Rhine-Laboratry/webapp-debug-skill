...
4. Spreadsheet IDがなければ、init時だけ新規作成を許可する。作成前にサービスアカウントと保存先を確認する。
5. `assets/google-sheets-schema.json` に従ってタブとヘッダーを作る。既存の未知列は削除しない。
6. 既存Playwright環境を検出する。なければ `tests/e2e/` に独立環境を作る案を提示し、許可された依存関係だけを追加する。
7. アプリケーション側の `package.json`、テスト、コードは変更しない。
8. DBガード項目が未設定なら、未設定キーだけを具体的に報告する。秘密値を表示しない。
9. init結果を表示して終了する。discover/testへ自動移行しない。

### discover

1. 共通Preflightとロック取得を行う。
2. `references/discovery.md` に従って静的探索を実施する。
...
10. 全Inventoryが `MAPPED` または `EXCLUDED_WITH_REASON` になるまで反復する。解析不能は `DISCOVERY_GAP` として未完了に残す。
11. 調査結果を要約し、ロックを解放する。
...
1. `discover` を完了条件まで実行する。
2. 調査完了を検証する。`DISCOVERY_GAP` が残る場合はtestへ移行せず、未完了として終了する。
3. 調査完了後、同じ実行コンテキストで `test` へ切り替える。
...
2. Inventory closure、Scenario生成率、PASS／FAIL／BLOCKED／FLAKY、Defect severity、cleanup異常、stale項目を表示する。
3. `DISCOVERY_GAP` がある場合、coverageを100%と表示しない。
4. ブラウザ起動、DB操作、テスト生成、Sheets更新を行わない。
