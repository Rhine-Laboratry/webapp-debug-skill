...
Google Sheets初期化は `scripts/init_sheets.py` を使う。CakePHP静的Inventory discoveryは `scripts/discover_cakephp_inventory.py` を使い、ローカルJSONへ出力してからcoverage gateや後続同期へ渡す。coverage gate評価は `inventory-json` 入力を使う。Sheetsの現在状態を評価する場合は、`scripts/export_sheets_snapshot.py` でread-only snapshotを作成してから `scripts/evaluate_coverage.py` へ渡す。snapshot exportはSheets write、lock、WAL、bootstrap、config更新を行わない。実Google Sheets API統合テストは明示envが揃った場合だけ実行する。Playwright Scenario生成器、ブラウザ実行を伴う動的discovery、Sheets syncは後続実装であり、現時点で実装済みと扱わない。
...
7. 既存Playwright環境を検出する。現時点ではPlaywright Scenario生成器は未実装なので、生成済みとして扱わない。
...
2. `references/discovery.md` と `references/discovery-rules.md` に従って静的探索を実施する。CakePHP静的Inventory候補は `scripts/discover_cakephp_inventory.py` でローカルJSON snapshotとして生成できる。
