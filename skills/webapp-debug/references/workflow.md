...
- Sheets反映前には `scripts/export_sheets_snapshot.py` でread-only snapshotを作成し、`scripts/plan_inventory_sync.py` でローカルsync planを生成する。適用する場合は `scripts/apply_inventory_sync.py --dry-run` でfresh snapshotとの整合を確認し、Spreadsheet ID完全一致確認、cooperative lock、WAL、read-backを通す。
...
- ローカルsync planningでは、discovery JSONとread-only Sheets snapshotを照合し、append/update/retire/gapの計画とconflictだけを出す。適用時も人間編集列は保持し、DELETE_ROWは作らない。
